...
def divisible_by_k(n, k):
    ...
    count = 0
    divisible_number = k
    while divisible_number <= n:
        if divisible_number % k == 0:
            count += 1
...
def double_eights(n):
    ...
    False
    """
    is_double_eights = False

    while n > 0:
        if n % 10 == 8 and is_double_eights:
            return is_double_eights
        elif n % 10 == 8 and n > 10:
            is_double_eights = True
        else:
            is_double_eights = False
        n = n // 10
    return is_double_eights
